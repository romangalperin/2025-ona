---
title: "2025 Organizational network analysis group project"
...
##### paths -----
data_path = "~/Library/CloudStorage/Dropbox/McGill/teaching/2024-2025/winter/ona/data/"
app_data_sample <- read_feather(paste0(data_path,"app_data_starter_coded_202502.feather"))
```
...
## Team project
Your team's goal is to perform the analysis of the agency's data as a consulting team would. Base on the analysis, you will write a short report (no longer than 10 pages including all figures table and references, if any). Your analysis will focus on the following questions:
...
Small data files are in the *CSV* format, larger ones are in *feather* format to reduce size. To read *CSV* files, use `readr` package that is part of the `tidyverse` package. To read *feather* files, use `arrow` package in R (or Python).
...
+ Only 9 relevant variables are included (see [PatEx data dictionary](https://www.uspto.gov/sites/default/files/documents/Appendix%20A.pdf))
...
Each record represents an instance of an examiner (`ego_examiner_id`) reaching out to another examiner (`alter_examiner_id`) for advice on a specific application (`application_number`).
